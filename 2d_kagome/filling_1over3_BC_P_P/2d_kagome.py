...
## kagome tight binding model --> see e.g.
## https://doi.org/10.1103/PhysRevB.68.195103
## https://doi.org/10.1103/PhysRevB.99.235118
## https://doi.org/10.1088/0953-8984/23/15/156002
def ene(orb,kx,ky):
    if orb==0:
        return 2.0
    elif orb==1:
        return - 1.0 - np.sqrt(1.0+8.0*np.cos(0.5*kx)*np.cos(0.5*ky)*np.cos(0.5*(kx-ky)))
    elif orb==2:
        return - 1.0 + np.sqrt(1.0+8.0*np.cos(0.5*kx)*np.cos(0.5*ky)*np.cos(0.5*(kx-ky)))
    else:
        return np.nan
...
#    filling_denom = 4 # quarter fill
    ...
    plt.savefig("fig_2d_kagome_enedens.png")
    plt.cla()
    plt.clf()
...
## fill numel > Lx*Ly
    if numel > 2*Lx*Ly:
        plt.plot(list_kx[2*Lx*Ly:numel],list_ky[2*Lx*Ly:numel],color='blue',marker='o',\
        ...
        plt.plot(list_kx[:numel],list_ky[:numel],color='blue',marker='o',\
            markerfacecolor='blue',linestyle='None')
    plt.savefig("fig_2d_kagome_fermisurface.png")
    plt.cla()
    plt.clf()
    ...
    plt.savefig("fig_2d_kagome_dos.png")
